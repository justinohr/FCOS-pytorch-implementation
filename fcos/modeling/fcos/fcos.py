...
class FCOS(nn.Module):
    ...
    def predict_proposals_single_image(
        ...
        for (cls_score, bbox_pred, centerness, points) in zip(
            ...
            scores = scores * centerness.unsqueeze(0).t()
            score, label = torch.max(scores, axis=1)
            #score = torch.sqrt(score * centerness)
            score = torch.sqrt(score)
            x_0 = (points[:,0] - bbox_pred[:,0]).unsqueeze(0)
            y_0 = (points[:,1] - bbox_pred[:,1]).unsqueeze(0)
            x_1 = (points[:,0] + bbox_pred[:,2]).unsqueeze(0)
            y_1 = (points[:,1] + bbox_pred[:,3]).unsqueeze(0)

            box = Boxes(torch.cat((x_0, y_0, x_1, y_1)).t())


            inst = Instances(image_size, pred_boxes=box, scores=score, pred_classes=label)
            bboxes_list.append(inst)
